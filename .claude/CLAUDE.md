...
- `client/src/burnscope_client/codex_daemon.py` — Long-lived daemon owning a `codex app-server` subprocess. Reader/pusher/health/poll coroutines; the pusher does fan-out across the codex paired list and advances `_last_pushed_snapshot` on any-device success; the poll loop calls `account/rateLimits/read` every 60 s and dedupes against `_last_pushed_snapshot` via `AgentSnapshot.semantically_equal` (the authoritative trigger, since the long-lived app-server doesn't see rate-limit changes from other `codex` CLI processes); the health loop probes each paired device and re-enqueues on divergence. Reconnects with exponential backoff on EOF.
