...
- `client/src/burnscope_client/agent.py` — `Agent` ABC. Every supported provider is a subclass. Defines `name`, `active_interval` / `idle_interval` (class attrs, seconds), `probe()`, `load_credential()`, and a default `try_create()` for the CLI's auto-detect path.
...
- `client/src/burnscope_client/daemon.py` — `DaemonConfig` takes `agents: list[Agent]`; each loop iteration resolves the per-agent cadence (`DaemonConfig.active_interval` / `idle_interval` are optional global overrides, otherwise the agent's own class attrs win), isolates per-agent failures, and pushes one `AgentSnapshot` per agent per cycle.
- `client/src/burnscope_client/cli.py` — `--agent` flag (repeatable). When absent, every agent whose credentials load is included. `--active-interval` / `--idle-interval` are optional global cadence overrides applied to every agent.
