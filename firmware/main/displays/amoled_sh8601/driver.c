...
/* SH8601/CO5300 address columns & rows in 2-pixel groups: every
 ...
 * a column. Mirrors the Waveshare BSP rounder. vres=466 is even, so the
 * 270° sw-rotation below preserves this parity, and the CO5300 x_gap of 6
 * (even) preserves it when the component adds it on each flush. */
static void sh8601_lvgl_rounder_cb(lv_area_t *area)
{
    area->x1 = (area->x1 >> 1) << 1;
    area->y1 = (area->y1 >> 1) << 1;
...
lv_display_t *amoled_sh8601_driver_init(void)
{
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .rounder_cb = sh8601_lvgl_rounder_cb,
