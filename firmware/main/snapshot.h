...
/**
 * Insert / overwrite the snapshot for `snap->agent`. Returns false if no
 * slot matches the agent name and none is free (cannot happen in MVP with
 * two agents and two slots).
 *
 * Invokes the registered listener after the slot is written.
 */
